# Beds24 Booking Page — Project Plan (2nd V, v2)
...
- Room card basic geometry (grid layout, card dimensions, image aspect ratio) is also included in critical CSS — when a guest arrives from the WordPress widget with dates pre-populated, room cards render immediately above the fold and will FOUC on slow connections if their structure is entirely in the external file
- External file handles remaining below-the-fold elements and all fine-grained styling
- On slow connections, guests see a styled booking strip and correctly laid-out room card skeletons immediately; the rest loads in
...
**Hide/reveal JS — timeout safeguard:** The timeout safeguard must only arm when `checkin` parameters are present in the URL — i.e., the guest arrived with dates pre-populated and rooms are expected to render. If the reveal event has not fired within 3 seconds in this case, room cards are force-shown. This prevents rooms from being permanently hidden if Beds24 updates their frontend and the event stops firing. When the page loads without date parameters (guest arrived directly), there is no timeout — the JS waits for the date selection event indefinitely, because there are no rooms to show until the guest enters dates.
...
| Hide/reveal room cards JS — silent failure risk | If Beds24 updates their frontend and the reveal event stops firing, rooms are permanently hidden with no visible error | Mitigated by timeout safeguard that arms only when `checkin` parameters are present in the URL: if reveal event has not fired within 3 seconds, room cards are force-shown. Without date parameters, no timeout — JS waits for date selection indefinitely. |
