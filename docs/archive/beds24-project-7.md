...
**Scope of state restoration:** The WordPress parent page restores the state it wrote — the dates and guest count as originally entered via the WordPress widget. It has no visibility into changes made inside the Beds24 iframe (same-origin policy). If a guest changes their dates inside the Beds24 UI and then refreshes, the parent will restore the original widget-entered dates, not the revised ones. This is a known and accepted limitation. Guests should be encouraged to use the WordPress widget to change dates rather than the controls inside the iframe.
