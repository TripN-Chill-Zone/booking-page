/*
 * Trip'N'Hostel Booking Widget v3
...
(function() {
  ...
  var css = ''
    ...
    + '.tnh-results-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;padding:0 2px}'
    + '.tnh-results-summary{font-size:14px;font-weight:500;color:var(--tnh-text-light)}'
  ...
  root.innerHTML = ''
    ...
    +       '<button type="button" class="tnh-results-close" id="tnh-results-close">Close</button>'
    +     '</div>'
  ...
  window.addEventListener('message', function(e) {
    ...
    if (data && data.type === 'tnh-height' && typeof data.height === 'number') {
      ...
      /* Hide loading spinner once rooms have rendered (height > 500px) */
      if (h > 500 && !roomsReady) {
        roomsReady = true;
        loadingEl.classList.remove('visible');
        iframeEl.style.display = 'block';
        sendScrollPos(); /* send initial position */
      }
    }
  });

  /* Send scroll position to iframe so it can position the floating bar */
  function sendScrollPos() {
    if (!iframeEl.contentWindow) return;
    var iframeRect = iframeEl.getBoundingClientRect();
    var viewportH = window.innerHeight;
    /* How far from the top of the iframe is the bottom of the viewport? */
    var visibleBottom = viewportH - iframeRect.top;
    /* Clamp: can't be negative (iframe above viewport) or beyond iframe height */
    visibleBottom = Math.max(0, Math.min(visibleBottom, iframeEl.offsetHeight));
    try {
      iframeEl.contentWindow.postMessage(
        JSON.stringify({type: 'tnh-scroll', visibleBottom: visibleBottom}), '*'
      );
    } catch(e) {}
  }

  /* Track scroll and resize on parent page */
  var scrollTimer;
  function onScroll() {
    clearTimeout(scrollTimer);
    scrollTimer = setTimeout(sendScrollPos, 30);
  }
  ...
  function showResults(cin, cout, nights, guests) {
    var summary = formatDateDisplay(cin) + ' \u2192 ' + formatDateDisplay(cout)
    ...
    var url = 'https://www.beds24.com/booking2.php'
      ...
      + '&numadult=' + guests
      + '&referer=widget'
      + '&cssfile=' + encodeURIComponent(CONFIG.cssfile);

    iframeEl.onload = function() {
      /* Don't hide spinner yet — wait for postMessage with real content height.
