# Beds24 Booking Page — Project Plan (2nd V, v3)
...
- Any core test fails → diagnose the widget configuration; if the Beds24 WordPress plugin cannot reliably pass parameters, build a custom HTML form widget in Kadence that constructs the Beds24 URL manually. This requires custom JavaScript on the WordPress side to parse the date inputs, calculate `numnight` from the check-in and check-out difference, format dates to Beds24's expected parameter format, and construct the full URL. It is a small JS task, not a no-code configuration.
...
- Above-the-fold layout (booking strip, page header) and room card basic geometry (grid layout, card dimensions, image aspect ratio) are replicated as Critical CSS in the Beds24 Custom CSS field (inline, no external request)
- Critical CSS contains only layout-shift-preventing properties: `display`, `grid`/`flex` structure, `width`, `height`, `aspect-ratio`, `min-height`. No colors, borders, typography, shadows, or spacing refinements — those load with the external file
- This is intentionally a duplication of a subset of the external file's structural rules. The tradeoff is accepted: a small maintenance cost (keeping inline and external in sync) in exchange for preventing FOUC on pre-populated arrivals where room cards are immediately above the fold
- External file handles all remaining styling
- On slow connections, guests see a correctly laid-out booking strip and room card grid immediately; fine styling loads in

**Critical CSS sync rule:** Any change to the external file that affects above-the-fold structure (booking strip or room card grid/dimensions) must be mirrored to all four properties' inline Custom CSS fields as part of the same deployment. This is an explicit step in the CSS update protocol below — it is not optional and must not be deferred.
...
2. If the change affects above-the-fold structure (booking strip or room card grid/dimensions), update the inline Critical CSS in one property's Beds24 Custom CSS field to match
3. Point that property's staging URL at the new file
4. Visually verify that property's booking page — booking strip, room cards, confirmation page, all viewport widths
5. Update inline Critical CSS and point staging URLs at the new file for all four properties
6. Visually verify all four staging environments before updating any production URL
7. Only after all four staging checks pass: update the production `&cssfile=` references and inline Critical CSS fields simultaneously

A fix that passes on Property 1 staging is not cleared for production until all four properties have been checked. However, pre-existing failures unrelated to the CSS change — such as a property with incorrect content entry or a room configuration issue — do not block the push. The verification step confirms the CSS change introduces no new breakage, not that every property is in a passing state for unrelated reasons.
...
**Hide/reveal JS — detection mechanism:** The primary reveal mechanism is a `MutationObserver` watching the room card container (e.g., `#b24scroller` or equivalent) for child nodes being added to the DOM. When room card nodes appear, the observer triggers the reveal and disconnects. This responds to what Beds24 actually renders rather than guessing when it will render. The observer only activates when `checkin` parameters are present in the URL — i.e., the guest arrived with dates pre-populated and rooms are expected. When the page loads without date parameters (guest arrived directly), the observer is not used — the JS waits for the date selection event indefinitely, because there are no rooms to show until the guest enters dates.

**Backstop timeout:** If the observer has been watching for 10 seconds and no room card nodes have appeared, room cards are force-shown. This is a last-resort safeguard for cases where Beds24's DOM structure has changed and the observer's target selector no longer matches. Ten seconds is long enough to accommodate slow mobile connections without firing prematurely.
...
**The live transaction test cannot be substituted with sandbox or test mode.** Sandbox environments do not reliably reproduce the confirmation page's CSS context. Budget one real transaction per property for this step. Notify the client's admin/accounting team before the test window — the transactions will appear in their payment gateway (e.g., Stripe) as real charges followed by immediate refunds, and gateway processing fees on refunded transactions are typically not returned.
...
**Ownership:** This architecture requires a developer to maintain. DOM-targeted CSS and JS will break when Beds24 updates their frontend, and fixing it requires someone who can inspect the DOM, author CSS, commit to git, and deploy via the staging protocol. This is not a task for hospitality staff. The client must have access to a web developer post-launch — whether that is a retainer with the original developer, an in-house resource, or another arrangement is the client's decision, but the dependency must be understood and agreed before launch.
...
| Hide/reveal room cards JS — silent failure risk | If Beds24 updates their frontend and the reveal event stops firing, rooms are permanently hidden with no visible error | Mitigated by `MutationObserver` watching for room card DOM nodes (primary mechanism) with a 10-second backstop timeout (last resort). Observer only activates when `checkin` parameters are present. Without date parameters, JS waits for date selection indefinitely. |
